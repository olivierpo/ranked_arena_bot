...
class AdminCog(commands.Cog):
  ...
  async def appoint_admin(self, ctx, admin_to_appoint: discord.Option(str)):
      trueskill_module.log_stuff(f"\n{ctx.command.qualified_name} -- {ctx.author.name} --" + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))
  ...
  async def remove_admin(self, ctx, admin_to_remove: discord.Option(str)):
      trueskill_module.log_stuff(f"\n{ctx.command.qualified_name} -- {ctx.author.name} --" + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))
      if ctx.author.id == 118201449392898052:
          admin_list = ""
  ...
  async def get_admins(self, ctx):
      ...
      admins = load_in_admins()
      if ctx.author.id in admins:
          await ctx.respond(admins, ephemeral=True)
      else:
          await ctx.respond("Unauthorized", ephemeral=True)

  @commands.command(name="add_player_admin", description="attaches json file", guild_ids=constants.GUILD_IDS)
  async def add_player_admin(self, ctx, player_name: discord.Option(str), player_mmr: discord.Option(float, required = False, default=constants.MMR_DEFAULT), mmr_confidence: discord.Option(float, required = False, default=constants.CONFIDENCE_DEFAULT)):
      trueskill_module.log_stuff(f"\n{ctx.command.qualified_name} -- {ctx.author.name} --" + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))
      admins = load_in_admins()
      if ctx.author.id in admins:
  ...
  async def update_player(self, ctx, player_name: discord.Option(str), player_mmr: discord.Option(float, required = False, default=-1), mmr_confidence: discord.Option(float, required = False, default=-1)):
      trueskill_module.log_stuff(f"\n{ctx.command.qualified_name} -- {ctx.author.name} --" + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()))
      admins = load_in_admins()
      if ctx.author.id in admins:
          pattern = re.compile("\\S+.*#\\S+")
